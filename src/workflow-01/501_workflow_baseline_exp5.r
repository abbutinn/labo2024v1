...
###experimento5, aumento las iteraciones de la BO a param_local$bo_iteraciones -> 150 
#modificamos -> scale_pos_weight = c(1L, 99L)
...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp/colaborativos/exp5/"
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    lambda_l1 = 0.0,#c(0.1, 2.0),#0.0, # lambda_l1 >= 0.0
    ...
    is_unbalance = FALSE, #
    scale_pos_weight = c(1L, 99L), #)1.0, # scale_pos_weight > 0.0
    ...
    num_leaves = c(8L, 2048L, "integer" ), #c(1000L, 2048L, "integer" ), #c( 300L, 1024L,  "integer" ), 
    min_data_in_leaf = c( 100L, 2000L, "integer" ), #c( 1000L, 10000L, "integer" ),
