...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp/test_guante/"
...
FE_historia_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$RandomForest$run <- TRUE #FALSE ##esto en true, mejora la ganancia.... en cualquier cosa que uno haga.
...
TS_strategy_guantesblancos_202109 <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$train$testing <- c(202107) ##aca es el testing, conozco la clase por eso puedo ver la ganancia 
  ...
  param_local$train$undersampling <- 0.1 #0.2 #(con 0.2 duplica el tiempo de la bayesiana)
...
TS_strategy_guantesblancos_202107 <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$train$training <- c(202103, 202102)#, 202101,202012, 202011, 202010, 202009, 202008, 202007)#c(202103, 202102, 202101)
  param_local$train$validation <- c(202104)
  param_local$train$testing <- c(202105)

  # Atencion  0.1  de  undersampling de la clase mayoritaria,  los CONTINUA
  # 1.0 significa NO undersampling ,  0.1  es quedarse con el 10% de los CONTINUA, 
  param_local$train$undersampling <- 0.1  ##duplicamos el tiempo al poner de 0.1 a 0.2 ( deberia ser 1.0)
  ###canaritos asesinos se aplican sabiendo que vamos a obtener menos ganancia... lo hacemos para tener mas experimentos.... 
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    lambda_l1 = c(0.1, 2.0),#0.0, # lambda_l1 >= 0.0
    lambda_l2 = 0.0, # lambda_l2 >= 0.0
    max_bin = 31L, # lo debo dejar fijo, no participa de la BO
    num_iterations = 9999, # un numero muy grande, lo limita early_stopping_rounds

    bagging_fraction = 1.0, # 0.0 < bagging_fraction <= 1.0
    pos_bagging_fraction = c(0.5,1.0),#1.0, # 0.0 < pos_bagging_fraction <= 1.0
    neg_bagging_fraction = 0.1, # 0.0 < neg_bagging_fraction <= 1.0
    is_unbalance = FALSE, #
    scale_pos_weight = c(0.5, 1.1), #)1.0, # scale_pos_weight > 0.0
    ...
    ####estoos son los mas importantes. con estos 4 esta bien. 
    learning_rate = c( 0.01, 0.8 ),
    feature_fraction = c( 0.5, 0.9 ),
    num_leaves = c(8L, 2048L, "integer" ), #c( 300L, 1024L,  "integer" ), 
    min_data_in_leaf = c( 10L, 10000L, "integer" ) # c( 100L, 2000L, "integer" )
  )


  # una Beyesian de Guantes Blancos, solo hace 15 iteraciones, necesitamos minimos 50!!!!!
  param_local$bo_iteraciones <-  5#50 # iteraciones de la Optimizacion Bayesiana
...
ZZ_final_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$kaggle$envios_hasta <- 11800L
  param_local$kaggle$envios_salto <-   200#500L

  # para el caso que deba graficar
  param_local$graficar$envios_desde <-  8000L
  param_local$graficar$envios_hasta <- 12000L #20000L
...
corrida_guantesblancos_202109( "gb01" ) ## comentamos si somos el grupo B. 


# Luego partiendo de  FE0001
# genero TS0002, HT0002 y ZZ0002

corrida_guantesblancos_202107( "gb02" ) ##comentamos si somos el grupo A
