...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp/agu_baseline_exp1/"
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    pos_bagging_fraction = c(0.1,1.0),#1.0, # 0.0 < pos_bagging_fraction <= 1.0
    neg_bagging_fraction = c(0.1,1.0), #0.1, # 0.0 < neg_bagging_fraction <= 1.0
    is_unbalance = FALSE, #
